...
#   dat_final %>% 
#   mutate(d_harsh_10 = ntile(d_harsh, 10)) %>%
#   filter(d_harsh_10 %in% c(1,10)) %>%
#   mutate(category = if_else(d_harsh_10 == 1, 'Fewest', 'Most')) %>% 
#   select(-contains('...'), 
#          -total, 
#          -contains('non_white'),
#          -school_name, 
...
#   district_plot + 
#   transition_states(states = category, transition_length = 2) + 
#   labs(title = 'Demographics of Districts with the {closest_state} 
# Harsh Punishments per Student') + 
#   theme(plot.title = element_text(hjust=0.5, size=rel(3)), 
#         #plot.caption = element_text(hjust=0.5, size=rel(3)), 
#         axis.text.y = element_text(size = rel(2)), 
...
#   exit_fade()
# 
# district_gif <- animate(district_animation, 
#                         nframes = 100, 
#                         height = 600, 
#                         width = 800, 
#                         start_pause = 20, 
...
#anim_save("district_gif.gif", district_gif)
```
