...
```{r, echo = F}
summary_1 <- dat_final %>% 
  mutate(`Quintile of % Black Students` = ntile(perc_black, 5)) %>% 
  group_by(`Quintile of % Black Students`) %>% 
  summarize(`Average % Black Students in Quintile` = mean(perc_black), 
...
summary_2 <- dat_final %>% 
  mutate(`Quintile of % FRL Students` = ntile(perc_frl, 5)) %>% 
  group_by(`Quintile of % FRL Students`) %>% 
  summarize(`Average % FRL Students in Quintile` = mean(perc_frl), 
           `Average Harsh Punishments per Student` = mean(harsh_ps))
...
```{r}
gridExtra::grid.arrange(school_plot, district_plot, heights = list(0.9, 1.0))
```
...
```{r}
plot(lmertree_TA, which = 'tree', ask = F)
```
### Figure 3. Second
...
#                   ids = "inner") 
# 
# ggsave('tree_plot_initial.png', temp, path = here::here('Data'))
tree_pic_initial <- png::readPNG(here::here('Data', 'tree_plot_initial.png'))
grid::grid.raster(tree_pic_initial)
```
