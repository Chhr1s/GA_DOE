...
agg_disc_prop_complete %>% 
  ...
  labs(y = 'Individual Districts', 
       x = 'District Average Harsh Punishments Per Student', #### MK: Plot title does not fit into the screen. Changed it a bit, but I guess you could also play with the text size.
       title = 'District Averages of Harsh Punishments per Student \nby Proportion of Black Students in District', 
       fill = 'Proportion
...
labeled_plot <- 
  ...
  geom_label(inherit.aes = F, 
            ...
            fill = 'coral') +
  scale_x_continuous(            ##### MK: Removed the empty space on the left side of the plot, which also sort of deals with
    expand = c(0, 0)             #####     the legend overlapping with labels.
  )
```
...
GA_sf <-                                    #### MK: For some reason, this code always returned an error message for me.
  lea_get(state = 'GA') %>%                 ####     I am sure it's about me, but because of this, I am not able to run the rest of the code.
  st_as_sf() %>% 
  mutate(district = 
