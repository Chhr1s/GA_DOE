...
GA_sf <- 
  lea_get(state = 'GA') %>% 
  st_as_sf() %>% 
  mutate(system_name = str_replace(string = NAME, pattern = " District", ""), 
         system_name = str_replace(string = system_name, pattern = " Schools", ""), 
         system_name = str_replace(string = system_name, pattern = " School", "")
         )
```
...
ms_oss <-
  disc_demo %>% 
  filter(fac_schtype == 'M') %>% 
  group_by(system_name) %>% 
  summarize(average_oss = mean(oss_adj), 
            average_prop_non_white = mean(prop_non_white))

hs_oss <-
  disc_demo %>% 
  filter(fac_schtype == 'H') %>% 
  group_by(system_name) %>% 
  summarize(average_oss = mean(oss_adj), 
            average_prop_non_white = mean(prop_non_white))
```
...
```{r, include = F}
ms_oss <- GA_sf %>% left_join(ms_oss, by = c('system_name'))
hs_oss <- GA_sf %>% left_join(hs_oss, by = c('system_name'))

```

```{r}
par(mfrow=c(1,2))
tm_shape(ms_oss) + tm_polygons('average_oss')
quartz()
tm_shape(ms_oss) + tm_polygons('average_prop_non_white')
```
