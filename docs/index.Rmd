...
Single imputation was used to fill missing data. The model shown here only reports group differences at the *p* < 0.001, after Bonferroni corrections, and with the minimum acceptable node as as 200 schools/node (i.e., just under 10% of sample). The model was estimated with restricted maximum likelihood (REML) with the `{glmertree}` package. 
...
**Middle and high schools with more than ~34% Black students** averaged just under **1 exclusionary punishment for every 3 students**. 
